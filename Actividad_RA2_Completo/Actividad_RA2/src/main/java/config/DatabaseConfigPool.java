...
public class DatabaseConfigPool {
    ...
    static {
        try {
            ...
            try (InputStream input = DatabaseConfigPool.class.getClassLoader().getResourceAsStream("config/db.properties")) {
                // Carga de las propiedades desde el fichero de configuración.
                // Se espera que 'db.url', 'db.user' y 'db.password' estén presentes.
                props.load(input);
            } catch (Exception ex) {
                throw new RuntimeException("No se pudo encontrar el archivo db.properties en config/");
            }
            ...
            throw new RuntimeException("Error al inicializar DatabaseConfigPool", e);
        }
    }
